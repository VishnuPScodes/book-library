...
export const Children = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get("https://uninterested-elk-cardigan.cyclic.app/children")
      .then((res) => {
        setData(res.data);
        setTimeout(() => {
  ...
  const handleSubmit = () => {
    ...
    axios
      .get(
        `https://uninterested-elk-cardigan.cyclic.app/children/search/${search}`
      )
      .then((res) => {
        setData(res.data);
      })
  ...
  return (
    <Div id="main-romance" theme={nightmode}>
      {loader == true ? (
        <ChildrenLoader />
      ) : (
        <div>
          <div id="header-pos">
            <Heading display={"flex"} textAlign={"center"}>
              {" "}
              <div
                onClick={() => {
                  navigate("/");
              ...
              <div style={{ margin: "auto",fontFamily:"Poppins" }}> Choose your book</div>
              ...
              <Tooltip label="Night mode">
                <Div2 theme={nightmode}>
                  ...
                  >
                    {nightmode == true ? <MoonIcon /> : <SunIcon />}
                  </Button>
                </Div2>
              </Tooltip>
            </Heading>
            ...
            >
              <div style={{ display: "flex" }}>
                ...
                />
                <Button id="submit-romance" onClick={handleSubmit}>
                  Submit
                </Button>
              </div>
              <div>
                {bookSearch == true ? (
                  ...
                  <div>{data.length} books found</div>
                ) : (
                  ""
                )}
              </div>
            ...
            <Menu>
              <MenuButtons id="floating-btn-roma" theme={nightmode}>
                <MenuButton
                  ...
                  as={Button}
                >
                  <Div3 theme={nightmode}>
                    {" "}
                    <ImBooks fontSize={"35px"} />{" "}
                  </Div3>
                </MenuButton>
              </MenuButtons>
              <MenuButtons>
            ...
            {data.length == 0
              ...
              : data.map((e) => {
                  return (
                    <Stack
                      onClick={() => {
                        navigate(`/ChildrenBookDetails/${e._id}`);
                      }}
                      marginTop={{ base: "40px", sm: "20px" }}
                      margin={{ base: "", sm: "auto" }}
                      marginLeft={{ base: "2.09%", sm: "" }}
                    >
                      {" "}
                      <ChildrenCard
